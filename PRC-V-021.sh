...
PRC-V-021	기술적 보안	OS 가상화 시스템	2. 시스템 서비스 관리	2. 콘솔 관리	DCUI 세션 타임아웃 설정	4	"사용자 부재시, 비인가자에 의한 시스템 무단 사용을 방지하기 위해, 일정시간 사용하지 않는 DCUI에 대한 세션 타임아웃(로그인 후 일정시간 사용하지 않을 경우 세션 종료) 설정의 적정성을 점검

* DCUI(Direct Console User Interface) : ESXi 호스트에 물리적으로 접근하여 사용할 수 있는 GUI 기반 관리 인터페이스"	-	ㅇ	-	-	"1. (방법1) SSH를 통해 ESXi 접속 후, 다음 명령어 실행 : Remote Host 확인
    $ vim-cmd hostsvc/advopt/view UserVars.DcuiTimeOut
2. (방법2) vSphere Client(ESXi) 접속 후, 다음 메뉴에 접근하여 확인(vSphere Client 버전에 따라, 메뉴 명칭은 달라질 수 있음)
    # (5.5) ""관리"" > ""설정"" -> ""시스템"" -> ""고급 시스템 설정"" > UserVars.DcuiTimeOut 설정 확인
    # (6.5/6.7/7.0) ""관리"" > ""시스템"" -> ""고급 시스템 설정"" > UserVars.DcuiTimeOut 설정 확인


"	"* 양호 : 세션 타임아웃 값(DcuiTimeOut)이 900 이하일 경우
* 취약 : 세션 타임아웃 값(DcuiTimeOut) 900 초과일 경우

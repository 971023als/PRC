...
PRC-V-013	기술적 보안	OS 가상화 시스템	2. 시스템 서비스 관리	1. 서비스 관리	SNMP Community 스트링 복잡성 미충족	3	SNMP Community 스트링이 유추하기 쉬운 문자열로 구성되어 있는 경우 비인가자가 SNMP 서비스에 무단 접근 가능할 수 있으므로, SNMP community string의 복잡성(최소길이, 문자종류) 요구사항 충족 여부를 점검	ㅇ	ㅇ	"if [[ $(vim-cmd proxysvc/service_list | grep 'TSM') ]]; then
...
SNMP Community 스트링 설정 확인

# /etc/snmpd.conf 파일에서 Community String 확인

# /usr/lib/vmware/vm-support/bin/nvpsvc -printconfig | grep -E ""SNMPCommunityString|SNMPAccessControl"""	"* 양호 - SNMP Community String 초기 값(Public, Private)이 아니고, 아래의 복잡도를 만족 할 경우
...
※ SNMP v3의 경우 별도 인증 기능을 사용하고, 해당 비밀번호가 복잡도를 만족할 경우 ""양호""로 판단"	"* 아래 방법을 통해 SNMP Community 스트링 설정(Communities) 확인

    1. (방법1) SSH를 통해 ESXi 접속 후, 다음 명령어 실행
       $ esxcli system snmp get"	"* 양호 - SNMP 서비스를 사용하지 않거나, SNMP Community String 초기 값(Public, Private)이 아니고, 아래의 복잡도를 만족할 경우
* 취약 - SNMP Community String 초기 값(Public, Private)이거나, 복잡도를 만족하지 않은 경우

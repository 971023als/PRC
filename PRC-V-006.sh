...
PRC-V-006	기술적 보안	OS 가상화 시스템	1. 인증 및 접근제어	2. 비밀번호 정책 설정	이전 비밀번호 재사용 요구사항 불충족	3	비밀번호 변경 시 이전 비밀번호 사용을 허가 할 경우, 비밀번호 추측, 이전 획득한 비밀번호 재사용 등의 위협이 발생 될 수 있으므로 이전 비밀번호 재사용을 제한하는 설정 존재 여부를 점검	ㅇ	ㅇ	"* 아래 방법을 통해 비밀번호 재사용 관련 설정을 확인
    ...
    1. (방법) vSphere Client 접속 후, 다음 메뉴에 접근하여 확인(vSphere Client 버전에 따라, 메뉴 명칭은 달라질 수 있음)
       ...
       # (vCenter8) ""관리"" > ""Single Sign On"" > ""구성"" > ""로컬 계정"" > ""암호정책"" > 비밀번호 재사용 관련 설정(재사용 제한) 확인"	"* 양호 - 비밀번호 재사용 횟수가 제한되어 있을 경우
* 취약 - 비밀번호 재사용 횟수가 제한되어 있지 않은 경우"	"* 아래 방법을 통해 비밀번호 재사용 정책 확인
...
※ 시스템이 관련 기능을 지원하지 않을 경우, 내부 정책 확인"	"* 양호 - 비밀번호 재사용 횟수가 제한되어 있을 경우

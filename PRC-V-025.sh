...
PRC-V-025	기술적 보안	OS 가상화 시스템	2. 시스템 서비스 관리	4. API	API 통신에 대한 보안 프로토콜 사용	3	API 통신 구간의 암호화가 적용되지 않을 경우, API 요청 내용이 위변조 될 수 있으므로, API 통신에 대한 보안 프로토콜(TLS) 적용 여부를 점검	ㅇ	ㅇ	"* 아래 방법을 통해 로그 설정 레벨을 확인

    1. (방법) vSphere Client 접속 후, 다음 메뉴에 접근하여 확인(vSphere Client 버전에 따라, 메뉴 명칭은 달라질 수 있음)
       # (vCenter6.5) ""호스트 및 클러스터"" > <vCenter 서버> > ""구성"" > ""설정"" > ""고급 설정"" >  ""VirtualCenter.VimApiUrl"" 확인
       # (vCenter8) ""호스트 및 클러스터"" > <vCenter 서버> > ""구성"" > ""설정"" > ""고급 설정"" >  ""VirtualCenter.VimApiUrl"" 확인"	"* 양호 - api URL이 HTTPS를 사용하는 경우
* 취약 - api URL이 HTTP를 사용하는 경우"	"* 아래 URL을 브라우저로 접속 후, 정상 작동 여부(http 상태에서 접속) 여부 확인

    http://[IP]/sdk/vimService.wsdl"	"* 양호 - api URL이 HTTPS를 사용하는 경우
